...
class ECGDataset(Dataset):
    ...
    def __init__(self, mode, data_dir, file_gen, file_sigs, file_bw, tvt_split, transform=None):
        assert mode in ['train', 'valid', 'test']
        self.mode = mode
    ...
    def __getitem__(self, idx):
        ...
        return sig_item + self.data_bw_[self.data_cuts_[2, idx]:self.data_cuts_[2, idx]+self.gen_sig_len], sig_item


def DataSplit(data_dir, file_gen, file_sigs, file_bw, tvt_split, batch_size=128, transform=None):
    ds_trn = ECGDataset('train', data_dir, file_gen, file_sigs, file_bw, tvt_split, transform)
    ds_val = ECGDataset('valid', data_dir, file_gen, file_sigs, file_bw, tvt_split, transform)
    ds_tst = ECGDataset('test', data_dir, file_gen, file_sigs, file_bw, tvt_split, transform)
